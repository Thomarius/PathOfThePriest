...
|---|---|---|
| `art/0-michi.png` | 0 — Michi | Generated by the project owner for this deck. No third-party licence applies. |
...
Nothing here needs clearing: the one image is the owner's own work, and
everything else is drawn in-repo. It is worth knowing that this is the only
image in either deck that is not generated by `motifs.js` / `decor.js`, so it is
the only thing a print service's upload screening could ever query.

Note that the validator only checks that this file *exists*, never that it is
accurate — a wrong entry here would not fail a build.
